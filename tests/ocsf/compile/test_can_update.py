...
def test_deep_allowed():
    assert _can_update(("attrs", "a"), None, 1, MergeOptions(allowed_fields=[("attrs", "a")])) == True
    assert _can_update(("attrs",), None, 1, MergeOptions(allowed_fields=[("attrs", "a")])) == False
    assert _can_update(("attrs", "a"), None, 1, MergeOptions(allowed_fields=[("attrs",)])) == True