...
from ocsf.compile.planners.profile import ExcludeProfileAttrsOp, MarkProfileOp, MarkProfilePlanner, ExcludeProfileAttrsPlanner, _find_profile
...
def get_repo():
    ...
    repo["extensions/linux/extension.json"] = DefinitionFile(
        "extensions/linux/extension.json",
        data=ExtensionDefn(name="linux", uid=1)
    )
    repo["extensions/windows/extension.json"] = DefinitionFile(
        "extensions/windows/extension.json",
        data=ExtensionDefn(name="win", uid=2)
    )
...
def test_find_profile():
    ...
    assert _find_profile(get_schema(), "win/ldap_stuff", "events/iam/account_change.json") == "extensions/windows/profiles/ldap_stuff.json"
